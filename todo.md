...
- [x] Engine `lotofacilPortfolio(type, targetNumbers, options)` em server/nexus-loterias.ts: gera 33 jogos de 15 dezenas cobrindo o alvo com pesos cognitivos (LSTM + estatístico + warmup + anomalias/entropia)
- [x] Camada de detecção de anomalias: paridade, soma, repetição do concurso anterior, dezenas de borda, sequências (heurísticas anti-padrão + detecção de anomalia estatística)
- [x] Portfólio persistente: tabela lotteryPortfolios (userId, lotteryType, games JSON, targetNumbers, createdAt, updatedAt, lastDrawChecked) — Migration 0023
- [x] Backend: conferir portfólio contra novo sorteio, evoluir pesos (reforço do que acertou 13+/acerto por jogo) e regenerar 33 jogos automaticamente
- [x] Histórico de evolução: tabela lotteryPortfolioEvolution (portfolioId, drawNumber, bestHits, hitsDist 11-15, generatedAt) + endpoint loterias.portfolioEvolution
- [x] Router: loterias.generatePortfolio (gerar/regenerar com evolução), loterias.checkPortfolio (conferir contra último sorteio + evoluir), loterias.listPortfolios, loterias.portfolioEvolution
- [x] Coleta automática Lotofácil (60 concursos) para alimentar o motor (dados reais Caixa)
- [x] Frontend: painel "Portfólio Lotofácil" na página /loterias — 15 dezenas-alvo clicáveis, 33 jogos em grade, metas 13/14/15, cobertura por dezena, dist de acertos, botão "Evoluir portfólio" (aplica novo sorteio)
- [x] Vitest coverage da Fase 29 (engine pura + conferência + evolução)
- [x] README + todo.md seção Fase 29, checkpoint (e30de11, em produção) + GitHub sync
